...
class SassSpec extends Specification {
  "sass parser" should {
    ...
    "parse a simple script" in {
      ...
      result.right.get must beEqualTo("div#main a, .sidebar p { font-size:10px; font-color:black; }\n")
    }
    ...
    "parse a script with nested ruleset" in {
      ...
      println(result)
      result.isRight mustBe true
      result.right.get must beEqualTo("table.h1 { margin:2em 0; }\ntable.h1 td.ln { text-align:right; }\n")
    }
    ...
    "parse a script with parent references in selectors" in {
      val result = Sass("div#main a, .sidebar p\n  font:\n    size: 10px\n  &:after\n    margin-top: 10px\n")
      result.isRight mustBe true
      result.right.get must beEqualTo("div#main a, .sidebar p { font-size:10px; }\ndiv#main a:after, .sidebar p:after { margin-top:10px; }\n")
    }
...
"""
      result.isRight mustBe true
      result.right.get must beEqualTo(expected)
    }
...
"""
    ...
    "parse a script with simple math expressions" in {
      val result = Sass("a\n  color: #123 + 20\n")
      result.isRight mustBe true
      result.right.get must beEqualTo("a { color:#253647; }\n")
    }
    ...
    "parse a script with calculated constants" in {
      ...
      result.isRight mustBe true
      result.right.get must beEqualTo("div#main a { font-size:23px; }\n")
    }
    ...
    "parse a script with calculated color constants" in {
      ...
      result.isRight mustBe true
      result.right.get must beEqualTo("div#main a { background-color:#253647; }\n")
    }
    ...
    "parse a script with complex calculated constants" in {
      ...
      result.isRight mustBe true
      result.right.get must beEqualTo("div#main a { border:2.5px solid #213243; }\ndiv#other { border:5px #324354 dashed; }\n")
    }
